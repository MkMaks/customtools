...
# # filter by Type Comments == stavebne upravy
	...
	if filterp and filterp.AsString() == "stavebne upravy":
		# setting Mark values
		# markSetter("Priemer","wc","Width","Height","wr")
		try:
			try:
				# circular windows
				diameter_param = window.LookupParameter('Priemer').AsDouble()
				# print("diameter = " +str(diameter_param))
				diameterValue = feet2mm(diameter_param)
				# wc = wall circular
				dimensionsList = [diameterValue,"wc"]
			except:
				# rectengular windows
				width_param = window.LookupParameter('Width').AsDouble()
				height_param = window.LookupParameter('Height').AsDouble()

				widthValue = feet2mm(width_param)
				heightValue = feet2mm(height_param)
				# wr = wall rectengular
				dimensionsList = [widthValue,heightValue,"wr"]
			# checking if it is known type
			if dimensionsList in windowTypes:
				index = windowTypes.index(dimensionsList)
				a = index + 1
			# if it is new type adding to the list of types
			else:
				windowTypes.append(dimensionsList)
				a = len(windowTypes)
...
for window in genericModel_collector:
# # filter by Type Comments == stavebne upravy
	elementTypeId = window.GetTypeId()
	elementType = Document.GetElement(doc,elementTypeId)
	filterp = elementType.LookupParameter('Type Comments')
	...
	if filterp and filterp.AsString() == "stavebne upravy":
		try:
			try:
				diameter_param = window.LookupParameter('Diameter').AsDouble()
				diameterValue = feet2mm(diameter_param)
				dimensionsList = [diameterValue,"gmc"]
			except:
				width_param = window.LookupParameter('Width').AsDouble()
				height_param = window.LookupParameter('Depth').AsDouble()

				widthValue = feet2mm(width_param)
				heightValue = feet2mm(height_param)

				dimensionsList = [widthValue,heightValue,"gmr"]
